...
class CkanConnector:
    ...
    def get_groups(self):
        ...
        try:
            if isinstance(result, str) and (result.startswith('local://') or result.startswith('file://') or os.path.isdir(result)):
                return True, []
        except Exception:
            pass
...
#                     dest_file += '.xml'
    ...
    def _validate_ckan_url(self, ckan_url):
        ...
        try:
            ...
            # plain directory paths (existing on disk) are local sources as well
            if isinstance(ckan_url, str) and os.path.isdir(ckan_url):
                return True, ckan_url
        except Exception:
